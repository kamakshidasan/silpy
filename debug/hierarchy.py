...
#     hierarchy_graph = tree_graph.reverse(copy=True) if reverse else tree_graph
#
#     # --- Group nodes by height with defaultdict ---
#     nodes_by_height = defaultdict(list)
#     for node in hierarchy_graph.nodes():
#         nodes_by_height[node.height].append(node)
#
#     # Build AGraph from the NetworkX graph
#     agraph = nx.nx_agraph.to_agraph(hierarchy_graph)
#     agraph.graph_attr.update(rankdir="TB", nodesep="0.3", ranksep="0.5")
#
#     # Map nodes to string names for Graphviz
#     node_to_name = {node: str(node) for node in hierarchy_graph.nodes()}
#
#     # For each height, pin nodes to same rank and chain by order
#     for nodes_at_height in nodes_by_height.values():
#         node_names_at_height = [node_to_name[node] for node in nodes_at_height]
#
#         # Anonymous subgraph (rank constraint only)
#         agraph.add_subgraph(node_names_at_height, rank="same")
#
#         nodes_sorted = sorted(nodes_at_height, key=lambda n: n.order)
#         for prev_node, next_node in zip(nodes_sorted, nodes_sorted[1:]):
#             agraph.add_edge(
#                 node_to_name[prev_node],
#                 node_to_name[next_node],
#                 style="invis",
#                 weight="100"
#             )
#
...
#
#     # Extract positions from Graphviz
#     positions = {}
#     for node, node_name in node_to_name.items():
#         position_string = agraph.get_node(node_name).attr.get("pos")
#         x_string, y_string = position_string.split(",")
#         positions[node] = (float(x_string), float(y_string))
#
#     node_pairs = [(branch.birth, branch.death) for branch in all_branches]
#
#     # Color each branch-pair, gray out up to 'step'
...
#         labels = {}
#         for branch in hierarchy_graph.nodes():
#             labels[branch] = ValueFormatter.format(branch.value)
#     elif label is not None and all(hasattr(branch, label) for branch in hierarchy_graph.nodes()):
#         labels = {}
#         for branch in hierarchy_graph.nodes():
#             attribute_value = getattr(branch, label)
...
#             labels[branch] = ValueFormatter.format(attribute_value)
#     else:
#         labels = {}
#         for branch in hierarchy_graph.nodes():
#             birth_string = branch.birth.label(label_key)
#             death_string = branch.death.label(label_key)
#             labels[branch] = f"({birth_string}, {death_string})"
#
#     color_list = [node_colors[node] for node in hierarchy_graph.nodes()]
#
#     plt.figure()
