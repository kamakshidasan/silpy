...
import numpy as np
from ..simplification.field_cutter import FieldCutter
from ..simplification.finder import Finder
...
class FieldSimplification:
    ...
    def update_patch_field_until_sufficient(self, removable_point, finder, patch_field):
        if removable_point.type == 'minimum':
            find_legal_extremum = finder.find_legal_minimum
            construct_field_from_extremum = FieldOrderer.compute_minimum_field
            find_complementary_extremums = finder.find_legal_maximums
            construct_field_from_complementary_extremums = FieldOrderer.compute_maximum_field

        elif removable_point.type == 'maximum':
            find_legal_extremum = finder.find_legal_maximum
            construct_field_from_extremum = FieldOrderer.compute_maximum_field
            find_complementary_extremums = finder.find_legal_minimums
            construct_field_from_complementary_extremums = FieldOrderer.compute_minimum_field
